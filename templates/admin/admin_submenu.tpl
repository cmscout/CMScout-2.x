...
{/literal}
document.location = "admin.php?page=subsite&subpage=submenu&action=delete&sid={$siteid}&id=" + articleId;
{literal}
}
...
<legend>{if $action == "new"}New{else}Edit{/if} Link</legend>
 <div class="field">
  <label class="label" for="name">Name<span class="hintanchor" title="Name of menu item."><img src="{$tempdir}admin/images/help.png" alt="[?]"/></span></label>
    <div class="inputboxwrapper"><input name="name" type="text" id="name" value="{$item.name}" class="inputbox" size="60" maxlength="50" onblur="checkElement('name', 'text', true, 0, 0, '');" /><br /><span class="fieldError" id="nameError">Required</span></div><br />
    
    <label class="label" for="items">Link to<span class="hintanchor" title="What does this menu item link to?"><img src="{$tempdir}admin/images/help.png" alt="[?]"/></span></label>
    <div class="inputboxwrapper"><select name="items" id="items" class="inputbox" onchange="itemss();">
     ...
     {/section}	
      ...
      {section name=groups loop=$numgroups}
        <option value="{$group[groups].id}.group" {if $item.item == $group[groups].id && $item.type == 3}selected="selected"{/if}>{$group[groups].teamname}</option> 
     {/section}  
</optgroup>     {/if}
    </select></div><br />
    
<div id="urldiv" style="display: {if $item.type == 5 || $item == ''}'';{else}none;{/if}">
    <label class="label" for="url">External Address<span class="hintanchor" title="The address of the external website."><img src="{$tempdir}admin/images/help.png" alt="[?]"/></span></label>
