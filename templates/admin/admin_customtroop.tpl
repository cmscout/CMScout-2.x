...
document.location = "admin.php?page=customtroop&action=delete&id=" + articleId;
}
//-->
  </script>
  {/literal}
...
<!--
 function changetype()
 {
    ...
    {
        case '1': case '2':
                optiondiv.innerHTML = '<label for="options" class="label">Maximum size</label><div class="inputboxwrapper"><input type="text" size="10" name="options" id="options" class="inputbox"/></div><br />';
                break;
        case '3': case '4': case '5':
        {/literal}
                optiondiv.innerHTML = '<label for="numoptions" class="label">Number of options</label><div class="inputboxwrapper"><input type="text" size="10" class="inputbox" name="numoptions" id="numoptions"  onchange="changeoptions(' + type + ')" value="1" style="width:70%"/><a href="#" onclick="takeone(' + type + ');"><img src="{$tempdir}admin/images/small_arrow_delete.png" title="[-]" border="0"/></a><a href="#" onclick="addone(' + type + ');"><img src="{$tempdir}admin/images/small_arrow_add.png" title="[+]" border="0"/></a></div><br /><div id="optiondiv2"><label for="option1" class="label">Option 1</label><div class="inputboxwrapper"><input type="text" name="option1" id="option1" size="50"  class="inputbox" style="width:70%"/></div><br /></div>';
                {literal}
                break;
        case '6':
                break;
 ...
 function changeoptions(type)
 {
    ...
    for(var i=1;i<=numoptions;i++)
    {
        ...
        html = html + '<label for="option' + i + '" class="label">Option ' + i + '</label><div class="inputboxwrapper"><input type="text" name="option' + i + '" id="option' + i + '" size="50"  class="inputbox" value="' + temp + '" style="width:70%"/>';
        if (i>1 && i < numoptions)
        {
           {/literal} 
...
<div class="field">
<label for="name" class="label">Name of field<span class="hintanchor" title="Required :: This is the name that CMScout uses internally to refer to the field. It can only contain alphanumeric characters and the underscore, and can not contain any spaces. It can not be changed once it has been set."><img src="{$tempdir}admin/images/help.png" alt="[?]"/></span></label>
<div class="inputboxwrapper">{if $action == "new"}<input type="text" name="name" id="name" size="50"  class="inputbox" {if $action=="edit"}value="{$item.name}"{/if}  onblur="checkElement('name', 'custom', true, 0, 0, /^[a-zA-Z0-9_]*$/);"/><br /><span class="fieldError" id="nameError">Required: May only contain alphanumeric characters and the underscore.</span>{else}{$item.name}{/if}</div><br />

<label for="query" class="label">Query<span class="hintanchor" title="Required :: This is the question that the user will see."><img src="{$tempdir}admin/images/help.png" alt="[?]"/></span></label><div class="inputboxwrapper"><input type="text" name="query" id="query" size="50"  class="inputbox" {if $action=="edit"}value="{$item.query}"{/if} onblur="checkElement('query', 'text', true, 0, 0, '');"/><br /><span class="fieldError" id="queryError">Required</span></div><br />

<label for="hint" class="label">Hint<span class="hintanchor" title="Required :: This is a explaination that will be shown to the user."><img src="{$tempdir}admin/images/help.png" alt="[?]"/></span></label><div class="inputboxwrapper"><input type="text" name="hint" id="hint" size="50"  class="inputbox" {if $action=="edit"}value="{$item.hint}"{/if} onblur="checkElement('hint', 'text', true, 0, 0, '');"/><br /><span class="fieldError" id="hintError">Required</span></div><br />

<span class="label">Required<span class="hintanchor" title="Required :: Is the user required to enter a value for this field."><img src="{$tempdir}admin/images/help.png" alt="[?]"/></span></span><span class="radiobox"><input type="radio" name="required" id="requiredyes" value="1" {if $item.required}checked="checked"{/if}/><label for="requiredyes">Yes</label>&nbsp;<input type="radio" name="required" id="requiredno" value="0" {if $action == "edit"}{if !$item.required}checked="checked"{/if}{else}checked="checked"{/if}/><label for="requiredno">No</label></span><br />

<span class="label">Type of user<span class="hintanchor" title="Required :: For which type of user is this field."><img src="{$tempdir}admin/images/help.png" alt="[?]"/></span></span><span class="radiobox"><input type="radio" name="register" id="registeryes" value="1" {if $item.register}checked="checked"{/if}/><label for="registeryes">{$scoutlang.members} only</label>&nbsp;<input type="radio" name="register" id="registerno" value="0" {if $action == "edit"}{if !$item.register}checked="checked"{/if}{else}checked="checked"{/if}/><label for="registerno">{$scoutlang.members} and Parents</label></span><br />

<span class="label">Visible<span class="hintanchor" title="Required :: Will this field be visible to the user in their user control panel."><img src="{$tempdir}admin/images/help.png" alt="[?]"/></span></span><span class="radiobox"><input type="radio" name="profileview" id="profileview:yes" value="1" {if $item.profileview}checked="checked"{/if}/><label for="profileview:yes">Show in User CP</label>&nbsp;<input type="radio" name="profileview" id="profileview:no" value="0" {if $action == "edit"}{if !$item.profileview}checked="checked"{/if}{else}checked="checked"{/if}/><label for="profileview:no">Hide from User CP</label></span><br />

<label for="type" class="label">Type<span class="hintanchor" title="Required :: What type of field is this."><img src="{$tempdir}admin/images/help.png" alt="[?]"/></span></label><div class="inputboxwrapper">
...
<div id="optiondiv">{if $action != "edit"}<label for="options" class="label">Maximum size<span class="hintanchor" title="Required :: Maximum size the user may enter. Enter zero for no size limit."><img src="{$tempdir}admin/images/help.png" alt="[?]"/></span></label><div class="inputboxwrapper"><input type="text" size="10" name="options" id="options"  class="inputbox"/></div><br />{else}
{if $item.type == 1 || $item.type == 2}
<label for="options" class="label">Maximum size<span class="hintanchor" title="Required :: Maximum size the user may enter. Enter zero for no size limit."><img src="{$tempdir}admin/images/help.png" alt="[?]"/></span></label><div class="inputboxwrapper"><input type="text" size="10" id="options" name="options"  class="inputbox" value="{$item.options}"/></div><br />
{elseif $item.type==3 || $item.type == 4 || $item.type==5}
<label for="numoptions" class="label">Number of options</label><div class="inputboxwrapper"><input type="text" size="10" class="inputbox" name="numoptions" id="numoptions"  onchange="changeoptions({$item.type})" value="{$item.options[0]}" style="width:70%"/><a href="#" onclick="takeone({$item.type});"><img src="{$tempdir}admin/images/small_arrow_delete.png" title="[-]" border="0"/></a><a href="#" onclick="addone({$item.type});"><img src="{$tempdir}admin/images/small_arrow_add.png" title="[+]" border="0"/></a></div><br />
<div id="optiondiv2">
{section name=options loop=$item.options[0]+1 start=1}
<label for="option{$smarty.section.options.iteration}" class="label">Option {$smarty.section.options.iteration}</label><div class="inputboxwrapper"><input type="text" name="option{$smarty.section.options.iteration}" id="option{$smarty.section.options.iteration}" size="50"  class="inputbox" value="{$item.options[options]}" style="width:70%"/>{if $smarty.section.options.iteration !=1}&nbsp;<a href="#" onclick="moveup({$smarty.section.options.iteration});" title="Move up"><img src="{$tempdir}admin/images/small_arrow_up.png" title="[^]" border="0"/></a>{/if}{if $smarty.section.options.iteration < ($item.options[0])}<a href="#" onclick="movedown({$smarty.section.options.iteration});" title="Move down"><img src="{$tempdir}admin/images/small_arrow.png" title="[v]" border="0"/></a>{/if}</div><br />
{/section}
</div>
{/if}
{/if}
